...
module.exports = async (req, res) => {
  try {
    ...
    // --- Helpers ---
    const toNYDate = (iso) => iso; // keep ISO; we’ll only adjust weekends
    const priorBizIfWeekend = (iso) => {
      const d = new Date(iso + "T00:00:00-04:00"); // NY offset ok for daily use
    ...
    // --- Run all in parallel ---
    const items = await Promise.all(symbols.map(s => fetchWSJ(s)));
